...
### (b) `\\b[a-z]{1,4}\\b`
This will detect a word begging with 1-4 characters and another word edge. In other words any word that is between 1 and four charters. Characters must be lowercase:
```{r}
...
### (d) `\\d{2}/\\d{2}/\\d{4}`
This looks at three two digit numbers divided by each other. This would be useful for dates. (I mean you could at least tell her you were could at programmming. Also it would work for chonological dates but ambiguity would be involved because America and the rest of the world right them differently.)
```{r}
...
### (e) `<(.+?)>.+?</\\1>`
A quick Google of the code gives some very weird results. The top hit was regarding altimeters. 
