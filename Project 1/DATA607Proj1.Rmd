...
#Two extracts are messy but it works.
opp <- as.integer(str_extract(str_extract(name,"\\d+\\|"), "\\d+")) # Average pre rating.
...
# found herehttps://stackoverflow.com/questions/15201305/how-to-convert-a-list-consisting-of-vector-of-different-lengths-to-a-usable-data
```
...
We now have a seperate dataframe with all the initial ratings of the opponents for each player.
We now need to get the average for each player.
...
```{r}
df <- data_frame(p_id, p_name, p_point, p_rate, opp, state)
